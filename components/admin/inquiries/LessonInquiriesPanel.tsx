...
const LAST_VIEWED_STORAGE_KEY = "admin-lesson-inquiries-last-viewed";
const READ_IDS_STORAGE_KEY = "admin-lesson-inquiries-read-ids";
...
function resolveRecordDate(inquiry: LessonsInquiry) {
...
function formatDateForDisplay(dateValue: Date | null | undefined) {
  ...
  try {
    ...
    console.warn("[LessonInquiries] failed to format date", error);
    return dateValue.toLocaleString();
  }
}

type InquiryDrawerProps = {
  inquiry: LessonsInquiry | null;
  onClose: () => void;
    ...
    <div className="fixed inset-0 z-50 flex">
      ...
      <div className="relative ml-auto h-full w-full max-w-xl overflow-y-auto border-l border-white/10 bg-zinc-950 p-6 shadow-2xl shadow-black/60">
        ...
        <div className="mt-6 grid gap-3">
          ...
          <DrawerField label="Name">{inquiry.name || "—"}</DrawerField>
          ...
          <DrawerField label="Time of Day">{inquiry.timeOfDay ?? "—"}</DrawerField>
...
type LessonInquiriesPanelProps = {
  firebase: Firebase;
};
...
export function LessonInquiriesPanel({ firebase }: LessonInquiriesPanelProps) {
  ...
  useEffect(() => {
    ...
    try {
      ...
      console.warn("[LessonInquiries] failed to read last-viewed state", error);
    }
  }, []);
  ...
  useEffect(() => {
    ...
    try {
      ...
      console.warn("[LessonInquiries] failed to read read-ids state", error);
    }
  }, []);
  ...
  useEffect(() => {
    ...
    try {
      ...
      emitAdminInquiryReadStateChanged("lesson");
    } catch (error) {
      console.warn("[LessonInquiries] failed to persist read-ids state", error);
    }
  }, [readIds]);
  ...
  useEffect(() => {
    ...
    try {
      ...
      console.warn("[LessonInquiries] failed to read view-mode state", error);
    }
  }, []);
  ...
  useEffect(() => {
    ...
    try {
      ...
      console.warn("[LessonInquiries] failed to persist view-mode state", error);
    }
  }, [viewMode]);
  ...
  const { inquiries, loading, error } = useLessonsInquiries(firebase, { refreshToken });
  ...
  const filteredInquiries = useMemo(() => {
    ...
    return inquiries.filter((inquiry) => {
      if (normalizedSearch) {
        const lookupTarget =
          `${inquiry.name} ${inquiry.email} ${inquiry.location ?? ""}`.toLowerCase();
  ...
  const handleMarkAllViewed = useCallback(() => {
    ...
    try {
      ...
      console.warn("[LessonInquiries] failed to persist last-viewed state", error);
    }
  }, [latestInquiryDate]);
  ...
  const updateWorkflow = useCallback(
    async (
      ...
      if (!inquiry) {
        console.warn("[LessonInquiries] attempted to update missing inquiry", inquiryId);
        return;
      }
      ...
      try {
        await updateDoc(doc(firebase.lessonsInquiriesRef(), inquiryId), payload);
      } catch (error) {
        console.error("[LessonInquiries] failed to update workflow", error);
      }
    },
    [firebase, inquiries],
  );
  ...
  const handleArchiveInquiry = useCallback(
    async (inquiryId: string) => {
      ...
      try {
        await updateDoc(
          doc(firebase.lessonsInquiriesRef(), inquiryId),
          { archivedAt: new Date().toISOString() },
        );
        setSelectedInquiryId(null);
      } catch (error) {
        console.error("[LessonInquiries] failed to archive", error);
      }
    },
    [firebase],
  );
  ...
  return (
    <div className="space-y-4">
      <div className="rounded-2xl border border-rose-500/40 bg-rose-500/15 px-4 py-3 text-sm text-rose-200">
        We couldn&apos;t load lesson inquiries. {error.message}
      </div>
      <button
        type="button"
        onClick={onRetry}
  ...
  return (
    <div className="rounded-2xl border border-white/10 bg-black/30 px-6 py-10 text-center text-sm text-white/60">
      {hasFilters
        ...
        : "No lesson inquiries have been submitted yet."}
    </div>
  );
}
