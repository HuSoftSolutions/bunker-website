...
import { useCareerInquiries, type CareerInquiry } from "@/hooks/useCareerInquiries";
...
const LAST_VIEWED_STORAGE_KEY = "admin-career-inquiries-last-viewed";
const READ_IDS_STORAGE_KEY = "admin-career-inquiries-read-ids";
const VIEW_MODE_STORAGE_KEY = "admin-career-inquiries-view-mode";
...
function resolveRecordDate(inquiry: CareerInquiry) {
...
function formatDateForDisplay(dateValue: Date | null | undefined) {
  ...
  try {
    ...
    console.warn("[CareerInquiries] failed to format date", error);
    ...
    <div className="fixed inset-0 z-50 flex">
      ...
      <div className="relative ml-auto h-full w-full max-w-xl overflow-y-auto border-l border-white/10 bg-zinc-950 p-6 shadow-2xl shadow-black/60">
        ...
        <div className="mt-6 grid gap-3">
          ...
          <DrawerField label="Position">{inquiry.position || "—"}</DrawerField>
          <DrawerField label="Skill Level">{inquiry.skillLevel || "—"}</DrawerField>
          ...
          <DrawerField label="Resume">
            {inquiry.resumeUrl ? (
              <a
                href={inquiry.resumeUrl}
                target="_blank"
                rel="noopener noreferrer"
                className="text-primary underline-offset-4 hover:underline"
              >
                {inquiry.resumeFileName || "View resume"}
              </a>
            ) : (
...
type CareerInquiriesPanelProps = {
  firebase: Firebase;
};

export function CareerInquiriesPanel({ firebase }: CareerInquiriesPanelProps) {
  ...
  const boardEnabled = settings.careersBoardEnabled;
  const boardStatuses = settings.careersBoardStatuses;
  ...
  useEffect(() => {
    ...
    try {
      ...
      console.warn("[CareerInquiries] failed to read last-viewed state", error);
    }
  }, []);
  ...
  useEffect(() => {
    ...
    try {
      ...
      console.warn("[CareerInquiries] failed to read read-ids state", error);
    }
  }, []);
  ...
  useEffect(() => {
    ...
    try {
      ...
      emitAdminInquiryReadStateChanged("career");
    } catch (error) {
      console.warn("[CareerInquiries] failed to persist read-ids state", error);
    }
  }, [readIds]);
  ...
  useEffect(() => {
    ...
    try {
      ...
      console.warn("[CareerInquiries] failed to read view-mode state", error);
    }
  }, []);
  ...
  useEffect(() => {
    ...
    try {
      ...
      console.warn("[CareerInquiries] failed to persist view-mode state", error);
    }
  }, [viewMode]);
  ...
  const { inquiries, loading, error } = useCareerInquiries(firebase, { refreshToken });
  ...
  const filteredInquiries = useMemo(() => {
    ...
    return inquiries.filter((inquiry) => {
      if (normalizedSearch) {
        const lookupTarget =
          `${inquiry.firstName} ${inquiry.lastName} ${inquiry.email} ${inquiry.location} ${inquiry.position}`.toLowerCase();
  ...
  const boardItems = useMemo<InquiryBoardItem[]>(() => {
    return filteredInquiries.map((inquiry) => ({
      ...
      subtitle: `${inquiry.position || "—"} • ${inquiry.location || "—"}`,
      meta: inquiry.skillLevel || undefined,
      contact: inquiry.email,
      dateLabel: formatDateForDisplay(resolveRecordDate(inquiry)),
      dateValue: resolveRecordDate(inquiry),
      laneValues: {
        location: inquiry.location || "—",
        source: inquiry.position || inquiry.skillLevel || "—",
      },
  ...
  const handleMarkAllViewed = useCallback(() => {
    ...
    try {
      ...
      console.warn("[CareerInquiries] failed to persist last-viewed state", error);
    }
  }, [latestInquiryDate]);
  ...
  const updateWorkflow = useCallback(
    async (
      ...
      if (!inquiry) {
        console.warn("[CareerInquiries] attempted to update missing inquiry", inquiryId);
        return;
      }
      ...
      try {
        await updateDoc(doc(firebase.careerInquiriesRef(), inquiryId), payload);
      } catch (error) {
        console.error("[CareerInquiries] failed to update workflow", error);
      }
    },
    [firebase, inquiries],
  );
  ...
  return (
    <div className="space-y-4">
      <div className="rounded-2xl border border-rose-500/40 bg-rose-500/15 px-4 py-3 text-sm text-rose-200">
        We couldn&apos;t load career inquiries. {error.message}
      </div>
      <button
        type="button"
        onClick={onRetry}
  ...
  return (
    <div className="rounded-2xl border border-white/10 bg-black/30 px-6 py-10 text-center text-sm text-white/60">
      {hasFilters
        ...
        : "No career inquiries have been submitted yet."}
    </div>
  );
}
      ...
      <table className="min-w-full divide-y divide-white/10 text-left text-sm text-white/80">
        ...
        <tbody className="divide-y divide-white/10">
          {inquiries.map((inquiry) => {
            ...
            const resumeLabel = inquiry.resumeFileName || "View resume";
            ...
            return (
              ...
              >
                <td className="px-4 py-3">
                  ...
                  <p className="mt-1 text-xs text-white/50">{inquiry.location || "—"}</p>
                </td>
                <td className="px-4 py-3">
