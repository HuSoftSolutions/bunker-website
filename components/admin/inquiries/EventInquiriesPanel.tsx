...
import { useEventsInquiries, type EventsInquiry } from "@/hooks/useEventsInquiries";
...
const LAST_VIEWED_STORAGE_KEY = "admin-event-inquiries-last-viewed";
const READ_IDS_STORAGE_KEY = "admin-event-inquiries-read-ids";
const VIEW_MODE_STORAGE_KEY = "admin-event-inquiries-view-mode";
...
function resolveRecordDate(inquiry: EventsInquiry) {
...
function formatDateForDisplay(dateValue: Date | null | undefined) {
  ...
  try {
    ...
    console.warn("[EventInquiries] failed to format date", error);
    return dateValue.toLocaleString();
  }
}

type InquiryDrawerProps = {
  inquiry: EventsInquiry | null;
  onClose: () => void;
      ...
      <div className="relative ml-auto h-full w-full max-w-xl overflow-y-auto border-l border-white/10 bg-zinc-950 p-6 shadow-2xl shadow-black/60">
        ...
        <div className="mt-6 grid gap-3">
          ...
          <DrawerField label="Phone">
            ...
            {inquiry.phoneExt ? (
              <span className="ml-2 text-white/60">ext. {inquiry.phoneExt}</span>
            ) : null}
          </DrawerField>
          <DrawerField label="Company">{inquiry.company || "—"}</DrawerField>
          <DrawerField label="Contact Preference">{inquiry.contactPreference}</DrawerField>
          <DrawerField label="Event Type">{inquiry.eventType}</DrawerField>
          <DrawerField label="Location">{inquiry.location}</DrawerField>
          <DrawerField label="Event Date">{inquiry.eventDate}</DrawerField>
          <DrawerField label="Time">
            {inquiry.startTime} – {inquiry.endTime}
          </DrawerField>
          <DrawerField label="Headcount">{inquiry.headcount}</DrawerField>
          <DrawerField label="Heard About Us">{inquiry.heardAbout}</DrawerField>
          <DrawerField label="Additional Info">
...
type EventInquiriesPanelProps = {
  firebase: Firebase;
};

export function EventInquiriesPanel({ firebase }: EventInquiriesPanelProps) {
  ...
  const boardEnabled = settings.eventsBoardEnabled;
  const boardStatuses = settings.eventsBoardStatuses;
  ...
  useEffect(() => {
    ...
    try {
      ...
      console.warn("[EventInquiries] failed to read last-viewed state", error);
    }
  }, []);
  ...
  useEffect(() => {
    ...
    try {
      ...
      console.warn("[EventInquiries] failed to read read-ids state", error);
    }
  }, []);
  ...
  useEffect(() => {
    ...
    try {
      ...
      emitAdminInquiryReadStateChanged("event");
    } catch (error) {
      console.warn("[EventInquiries] failed to persist read-ids state", error);
    }
  }, [readIds]);
  ...
  useEffect(() => {
    ...
    try {
      ...
      console.warn("[EventInquiries] failed to read view-mode state", error);
    }
  }, []);
  ...
  useEffect(() => {
    ...
    try {
      ...
      console.warn("[EventInquiries] failed to persist view-mode state", error);
    }
  }, [viewMode]);
  ...
  const filteredInquiries = useMemo(() => {
    ...
    return inquiries.filter((inquiry) => {
      if (normalizedSearch) {
        const lookupTarget =
          `${inquiry.firstName} ${inquiry.lastName} ${inquiry.email} ${inquiry.company ?? ""} ${inquiry.eventType} ${inquiry.location}`.toLowerCase();
  ...
  const boardItems = useMemo<InquiryBoardItem[]>(() => {
    return filteredInquiries.map((inquiry) => ({
      ...
      subtitle: inquiry.eventType || "Event Inquiry",
      meta: `${inquiry.eventDate || "—"} • ${inquiry.location || "—"}`,
      contact: inquiry.email,
      dateLabel: formatDateForDisplay(resolveRecordDate(inquiry)),
      dateValue: resolveRecordDate(inquiry),
      laneValues: {
        location: inquiry.location || "—",
        source: inquiry.heardAbout || "—",
      },
  ...
  const handleMarkAllViewed = useCallback(() => {
    ...
    try {
      ...
      console.warn("[EventInquiries] failed to persist last-viewed state", error);
    }
  }, [latestInquiryDate]);
  ...
  const updateWorkflow = useCallback(
    async (
      ...
      if (!inquiry) {
        console.warn("[EventInquiries] attempted to update missing inquiry", inquiryId);
        return;
      }
      ...
      try {
        await updateDoc(doc(firebase.eventsInquiriesRef(), inquiryId), payload);
      } catch (error) {
        console.error("[EventInquiries] failed to update workflow", error);
      }
    },
    [firebase, inquiries],
  );
  ...
  const handleArchiveInquiry = useCallback(
    async (inquiryId: string) => {
      ...
      try {
        await updateDoc(
          doc(firebase.eventsInquiriesRef(), inquiryId),
          { archivedAt: new Date().toISOString() },
        );
        setSelectedInquiryId(null);
      } catch (error) {
        console.error("[EventInquiries] failed to archive", error);
      }
    },
    [firebase],
  );
  ...
  return (
    <div className="space-y-4">
      <div className="rounded-2xl border border-rose-500/40 bg-rose-500/15 px-4 py-3 text-sm text-rose-200">
        We couldn&apos;t load event inquiries. {error.message}
      </div>
      <button
        type="button"
        onClick={onRetry}
  ...
  return (
    <div className="rounded-2xl border border-white/10 bg-black/30 px-6 py-10 text-center text-sm text-white/60">
      {hasFilters
        ...
        : "No event inquiries have been submitted yet."}
    </div>
  );
}
          ...
          {inquiries.map((inquiry) => {
            ...
            return (
              ...
              >
                ...
                <td className="px-4 py-3">
                  <p className="text-sm text-white">{inquiry.eventType}</p>
                  <p className="text-xs text-white/60">{inquiry.heardAbout}</p>
                </td>
                ...
                <td className="px-4 py-3 text-sm text-white">
                  {inquiry.headcount ? `${inquiry.headcount}` : "—"}
                </td>
                <td className="px-4 py-3">
                  <WorkflowControls
                    inquiryId={inquiry.inquiryId}
