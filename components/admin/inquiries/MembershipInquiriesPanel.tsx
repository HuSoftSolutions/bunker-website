...
const LAST_VIEWED_STORAGE_KEY = "admin-membership-inquiries-last-viewed";
const READ_IDS_STORAGE_KEY = "admin-membership-inquiries-read-ids";
...
export function MembershipInquiriesPanel({ firebase }: MembershipInquiriesPanelProps) {
  ...
  useEffect(() => {
    ...
    try {
      ...
      console.warn("[MembershipInquiries] failed to read last-viewed state", error);
    }
  }, []);
  ...
  useEffect(() => {
    ...
    try {
      ...
      console.warn("[MembershipInquiries] failed to read read-ids state", error);
    }
  }, []);
  ...
  useEffect(() => {
    ...
    try {
      ...
      emitAdminInquiryReadStateChanged("membership");
    } catch (error) {
      console.warn("[MembershipInquiries] failed to persist read-ids state", error);
    }
  }, [readIds]);
  ...
  const handleMarkAllViewed = useCallback(() => {
    ...
    try {
      ...
      console.warn("[MembershipInquiries] failed to persist last-viewed state", error);
    }
  }, [latestInquiryDate]);
